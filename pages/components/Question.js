...
const Question = ({questionText, options, answer, handleQuestion, handleScore, handleWrongAnswer}) => {
      ...
      <div className="flex flex-col justify-center h-[80vh]">
        <div className="lg:w-6/12 w-10/12 mx-auto px-10 py-16 outline outline-slate-300 outline-2 rounded-3xl shadow-xl">
          ...
          {options.map((option, index) => {
            return(
              <p className=" font-medium drop-shadow-sm my-2 bg-skin-main text-white p-3 rounded-xl pl-5 hover:bg-teal-700 transition-all cursor-pointer" onClick={async ()=> {await handleAnswer(option); handleQuestion()} }>
        {option}
      </p>
            )
          })}
        ...
        <br />
        {isCoreect === null && (
          <p  className=" font-medium drop-shadow-sm ml-3">Let's Start Evaluating!</p>
        )}
        {isCoreect !== null && (
          <>
        {isCoreect ? (
